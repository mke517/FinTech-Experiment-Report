...
This study conducted a systematic analysis of the functional architecture and execution logic of an LLM-based multi-agent financial market simulation framework. We dissected the six core components of the system (Simulator, Persona, Proxy, Topology, Communication) and their coordinated execution pipeline, elucidating how each module contributes to the closed loop of perception, decision-making, action, and feedback in a distributed multi-agent environment.

Through an empirical case study on asset bubble simulation, we compared the runtime behaviors of the RAG model and the LLM model. Both models successfully reproduced the formation phase of asset bubbles, exhibiting significant price deviation from fundamental values and pronounced overvaluation dynamics. However, neither model achieved complete bubble bursting: the RAG model produced a pattern of ``long-term overvaluation with gradual reversion,'' where prices remained persistently above fundamentals without rapid collapse; the LLM model exhibited recurrent cycles of ``surge, retracement, and resurge,'' with overvaluation states repeatedly re-emerging after drawdowns, preventing thorough bubble dissipation.

These findings reveal that while current LLM-driven multi-agent systems can generate realistic bubble formation dynamics, their parameter calibration and agent behavioral mechanisms remain insufficient for faithfully replicating the complete lifecycle of asset bubbles, particularly the rapid liquidation and mean-reversion phases predicted by canonical financial theory. The primary limitations identified include insufficient sensitivity of bubble bursting trigger conditions, weak fundamental-based correction mechanisms, and imbalanced trader structure configurations.

This work provides a foundational architectural understanding and empirical baseline for future research on interpretable financial market simulation. The insights gained lay the groundwork for refining parameter precision, enhancing agent heterogeneity, and ultimately constructing an interactive financial sandbox model that offers transparent, explainable decision-making processes for financial practitioners and researchers.
